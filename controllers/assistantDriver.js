...
router.post("/", async (req, res) => {
  try {
    const newAssistantDriver =
      await assistantDriverService.createAssistantDriver(
        req.body.user_id,
        req.body.vehicles,
      );
...
router.get("/:assistantDriverId", async (req, res) => {
  try {
    const assistantDriver = await assistantDriverService.getAssistantDriverById(
...
router.put("/:assistantDriverId", async (req, res) => {
  try {
    const updatedAssistantDriver =
      await assistantDriverService.updateAssistantDriver(
        req.params.assistantDriverId,
        req.body,
      );
...
router.delete("/:assistantDriverId", async (req, res) => {
  try {
    await assistantDriverService.deleteAssistantDriver(
      req.params.assistantDriverId,
    );
...
router.get("/", async (req, res) => {
  try {
    const assistantDrivers =
      await assistantDriverService.getAllAssistantDrivers();
...
router.get("/by-vehicle/:vehicleId", async (req, res) => {
  try {
    const assistantDrivers =
      await assistantDriverService.getAssistantDriversByVehicleId(
